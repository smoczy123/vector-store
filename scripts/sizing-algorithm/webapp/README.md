...
The sizing algorithm is versioned via the `ALGORITHM_VERSION` constant in
`vss_sizing.py`.  Bump this integer whenever formulas or heuristics change
in a way that would produce different results for the same inputs.

* The `/api/compute` response includes `algorithm_version` so that API
  consumers can detect when recommendations were generated by a different
  version.
* Shareable URLs embed the version as a `v=` query parameter.  When a link
  generated with an older version is opened, the webapp displays a warning
  banner informing the user that results may differ from the original
  estimate.
