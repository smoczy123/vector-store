...
async fn create_index(
    ...
    indices.insert(index.to_string(), Index::new(&options).unwrap());
...
async fn remove_document(
    ...
    let mut indices = state.indices.write().unwrap();
    if let Some(index) = indices.get_mut(&index) {
...
async fn get_count(
    ...
    if let Some(index) = indices.get(&index) {
        return (StatusCode::OK, Json(json!({ "count": index.size() })));
    }
    (
        StatusCode::NOT_FOUND,
        Json(json!({ "error": "index not found" })),
